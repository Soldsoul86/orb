...
Run with `npm test` (Node's built-in `node:test`, no framework). 95 tests.
...
| From the first real import — `test/realformats.test.ts` | AU Bank IMPS, UPI and interest credits · Canara "shall be debited" is a notice · promotional (-P) senders ignored · promo phrases not taken as autopay merchants · hashed autopay IDs shown as unnamed · autopays silent 60+ days are dormant · empty export reported · all unread alerts listed, redacted |
