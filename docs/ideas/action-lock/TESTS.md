...
Run with `npm test` (Node's built-in `node:test`, no framework). 153 tests, plus 9 Kotlin unit tests in `android/` (`./gradlew testDebugUnitTest`).
...
## Pay guard — `test/guard.test.ts`, `android/app/src/test/…/GuardRulesTest.kt`, `PayScreenTest.kt`
...
| Parity | The Kotlin guard decides exactly as the TypeScript one on every shared case (`test/guard-vectors.json`, regenerated by `npm run guard-vectors`; the TS test fails if it is stale). A one-character change to the Kotlin rule was made and the test caught it |
| UPI PIN screen | The NPCI PIN screen as logged from Google Pay and PhonePe: amount from "Pay ₹3000.00", payee from "To …" (a name or a UPI ID, never "Close"), the whole keypad and submit covered; a PIN screen showing only a UPI ID is matched by that ID, or by the name seen with it on the screen before |
