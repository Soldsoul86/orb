...
| **G5 — Hibernation** | **Observed 2026-09-25, untested.** *Manage app if unused* is **on** by default: *"Remove permissions, delete temporary files, stop notifications and archive the app"*, and *"permissions for this app will be removed if the app is unused for a few months."* An always-on recorder is precisely an app nobody ever opens. Whether a running foreground service counts as use is **unknown**, and unlike every other gate here it **cannot be settled by a day of measurement** — the threshold is months. The first gate the device loop cannot reach, which is a limit of the method and is recorded as one. |
| **G4a — Boot** | **Measured 2026-09-25 (`DEVICE_LOOP.md` §5k):** `dataSync` is refused by name at `BOOT_COMPLETED` — *"FGS type dataSync not allowed to start from BOOT_COMPLETED!"* — while **`specialUse` reaches the foreground 133 s after boot and records with nobody present.** The always-on path exists. Its cost is not technical: `specialUse` needs a written justification in the manifest, which the Play Store reviews and a sideloaded build does not, so this may be available to an operator and not to a distributed app. |
| **G4 — Power and quota** | Doze, App Standby buckets, and — since Android 15 — a **6-hour-per-24h cap** on `dataSync` and `mediaProcessing` foreground services, tracked separately per type. **Measured 2026-09-25 (`DEVICE_LOOP.md` §5h): the cap did not fire on this device, on a default-configured app.** `dataSync` ran 6h31m untouched with battery usage set to **Optimised** — the default — and no exemption granted, so the result is not an artefact of configuration. It cost **0% of battery**. Doze did defer, heavily — 47 deferrals over 90 s, the longest 9.6 min — so the **quota** half of this row did not bite and the **power** half very much did. Android 16 further subjects jobs started from a foreground service to their own runtime quotas, including `WorkManager`. | A "continuous" runtime that stops at hour six and cannot restart itself until the user opens the app. |
