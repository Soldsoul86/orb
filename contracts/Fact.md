...
in history. The difference between a Fact and a Belief is *stance*, not kind: a
Fact is relied upon as settled; a Belief is held with explicit uncertainty. Both
are grounded in Evidence and recomputable.
...
4. **Revision (never rewrite).** Contradicting Evidence can unsettle the Fact; a
   new value is appended, the prior value retained. A Fact may also *demote* to a
   Belief if its grounding weakens, or be *retired* when superseded.
...
(settled) ──relied upon──▶ (referenced)
    ...
    └──grounding weakens──▶ (demoted to Belief)
```
...
4. **Revision is append-only.** Updating a Fact retains its prior value; history is
   never rewritten.
5. **Knowledge plane only.** A Fact holds no power to act and no source-of-truth
   state of its own.
...
- New **Fact categories** (contact detail, location, schedule, preference, …) are
  added freely and versioned; existing categories never change meaning.
- The core obligation — *evidence-grounded, entity-scoped, settled-until-
  contradicted, revised by append* — is frozen at v1.
...
- Every Fact is traceable, through Evidence, to recorded history, and scoped to an
  Entity.
- A Fact may be revised or demoted, but its prior values are retained.
...
- **Contradicted Fact.** New Evidence contradicts a settled Fact; the Fact is
  revised (or demoted to Belief), prior value retained, history untouched.
- **Over-settlement.** A claim treated as settled on thin Evidence is demoted when
  its grounding is re-examined; the demotion is recorded.
...
Evidence; rewriting a prior Fact value; a Fact with no Entity.
...
- **Demotion.** "The user prefers morning meetings," once a Fact, accumulates
  contradicting Evidence and is demoted to a Belief held with lower confidence.
